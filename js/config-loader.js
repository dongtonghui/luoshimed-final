...
function renderHomePage() {
  ...
  if (hero) {
    ...
    if (hero.trustBar && Array.isArray(hero.trustBar) && hero.trustBar.length >= 3) {
      const trustItems = document.querySelectorAll('.hero-trust-bar .trust-text');
  ...
  // 核心优势区 - 禁用动态渲染，使用纯静态HTML避免bfcache问题
  // 静态内容已在 index.html 中定义，不再通过JavaScript修改
  // 这样可以避免从bfcache恢复时DOM元素状态不一致的问题
